...
import { fromFils, pct, roundFils } from './money.js';
export function buildWaterfall(listPrice, selection, floor) {
    const waterfall = [];
    ...
    const premiumUplift = POSTURE[selection.posture].premiumUplift ?? 0;
    if (premiumUplift > 0) {
    ...
    const foundingApplied = Math.min(foundingRequested, selection.maxFoundingDiscount, FOUNDING_MAX_DISCOUNT_PERCENT);
    pushDiscount(waterfall, 'founding', 'Founding-client discount', foundingApplied, listPrice, `Requested ${foundingRequested}%, capped at ${foundingApplied}%`, foundingRequested);
...
function pushDiscount(waterfall, step, label, appliedPercent, listPrice, reason, requestedPercent = appliedPercent) {
    if (appliedPercent <= 0 && requestedPercent <= 0)
        return;
...
function enforceFloor(listPrice, waterfall, floor) {
    ...
    if (net < floor) {
        ...
        for (let i = waterfall.length - 1; i >= 0 && deficit > 0; i -= 1) {
            ...
            if (!step || step.amount >= 0 || (!step.discountLike && step.step !== 'tier'))
                continue;
...
function sumSigned(listPrice, waterfall) {
