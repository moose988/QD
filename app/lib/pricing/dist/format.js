...
export function formatEstimateText(estimate, { businessName = '' } = {}) {
    ...
    if (estimate.approval)
        out.push(`Approval: ${estimate.approval}${estimate.flags?.length ? ` (${estimate.flags.join(', ')})` : ''}.`);
    if (Number.isFinite(estimate.marginPercent))
        out.push(`Internal margin: ${estimate.marginPercent}%.`);
    if (estimate.floorBound)
        out.push('Cost floor applied: discount was capped to protect minimum gross margin.');
    return out.join('\n');
}
export function threeLineClientView(result) {
